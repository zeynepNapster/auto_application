...
filtered_dfs_1=filtered_dfs.groupby('files').content.apply(str).pipe(lambda d: d[d.str.lower().str.contains(r'\belectrical engineer\b')])
filtered_dfs_2=filtered_dfs.groupby('files').scores.mean().nsmallest(120)


filtered_dfs_1['files']=filtered_dfs_1.reset_index().files.str.replace('texts_eng','jsons').str.replace('.txt','.json')
filtered_dfs_2['files']=filtered_dfs_2.reset_index().files.str.replace('texts_eng','jsons').str.replace('.txt','.json')

for k,linked_in_card in enumerate(list(set(filtered_dfs_1.files.tolist()+filtered_dfs_2.files.tolist()))):#


    k=linked_in_card.split('/')[-1]
    pattern = r'\d+'

    # Find all integers in the script
    integers = re.findall(pattern, k)




    file = load_json('./'+linked_in_card)
    name = filter_alphabetic(file['h1_headers'])
    if os.path.exists(os.path.join(f'./applications/application_{k}/{name}_application.txt')):
        continue
